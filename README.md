...
- **Move, reorder, sort** — drag a card to another column to change status, or reorder within a column (written to a configurable `manual_order` property). Or auto-sort each column by **name** or any **property** — including a **base formula** like a `priority_score` — ascending/descending. Full **keyboard** support (move, reorder, menu) and command-palette commands.
...
- **Cards show your view's properties** — each card renders the properties you've added to the Bases view (the standard **Properties** selection), labelled and in order — ordinary properties **and base formulas** (`formula.…`) alike, so a computed value shows with no extra setup.
