...
- **Timeline mode.** A Gantt-style view: one row per card, a bar spanning its start → end dates (configurable properties, defaulting to scheduled/due), milestone diamonds from a configurable list property (`milestones: [2026-09-01 Beta]`), a today line, and drag-to-reschedule that preserves the duration. Week/Month/Quarter/Year axis, undated cards in a collapsible strip.
