...
- **WBS mode.** A work-breakdown-structure view rendering your parent/child hierarchy as a collapsible tree — goal → projects → tasks — with per-node progress bars, estimates, and date spans. One rollup model: a note's own value wins, otherwise it derives from its children — so you can plan top-down, bottom-up, or both, and persist any rollup to the parent in one click (never double-counted). Drag rows to re-parent (the relationship wikilink is rewritten for you), drag cards from the "Needs planning" backlog into the tree, and distribute a parent's estimate across unestimated children.
