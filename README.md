...
- **Timeline mode.** A Gantt-style view: one row per card, a bar spanning its start → end dates (configurable properties, defaulting to scheduled/due) with a duration badge (`12d`), milestone diamonds from a configurable list property (`milestones: [2026-09-01 Beta]`), a today line, drag-to-reschedule that preserves the duration, edge handles to resize just the start or end, and drag-down-onto-the-Undated-strip to unschedule. Week/Month/Quarter/Year axis with Ctrl/Cmd+wheel zoom anchored under the cursor, undated cards in a collapsible strip.
