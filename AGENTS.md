...
- **Every view mode is optimistic (hard rule).** An edit made in a mode (drag, resize, key, menu) shows its result IMMEDIATELY from in-memory state, and the frontmatter write happens in the background; the Base's re-query must never be what makes the change visible. Two halves, both required: (1) an in-memory overlay or in-place mutation that renders read through until the vault echoes the same value back (drop it on echo, and after a grace period so a linter rewrite cannot pin it; on a failed write, drop it and re-render), and (2) a renderer that PATCHES the existing DOM by key (signature-gated per element: unchanged elements keep their node, focus, hover and selection) instead of emptying and rebuilding, so the echo causes no flash or reflow. Board (`reconcile.ts` + `signatures.ts`), WBS (`rowSignature`, in-place re-parent), and Ideal week (`overlay` + `reconcilePieces`) are the references; a new mode copies one of them. Business rule 49.
